...
# 修改后的第二个绘图函数：更改图例名称
# ==========================================
def plot_projection_comparison(p0, p1, p2, p3, p4, title):
    fig, axes = plt.subplots(1, 3, figsize=(14, 5), sharex=False, sharey=False)
    
    # SCI 常用配色 (保持不变，用于内部引用颜色)
    sci_colors = {
        "Original": "black",
        "S-G": "#1f77b4",
        "Spline": "#d62728",
    ...
    def draw(ax, x_idx, y_idx, xlabel, ylabel, collect_legend=False):
        # 修改这里的 label 名称
        l0, = ax.plot(p0[:,x_idx], p0[:,y_idx], color=sci_colors["Original"], label="Original", linewidth=1.5, alpha=0.8)
        l1, = ax.plot(p1[:,x_idx], p1[:,y_idx], color=sci_colors["S-G"], label="Method 1", linewidth=1.5)
        l2, = ax.plot(p2[:,x_idx], p2[:,y_idx], color=sci_colors["Spline"], label="Method 2", linewidth=1.5)
        l3, = ax.plot(p3[:,x_idx], p3[:,y_idx], color=sci_colors["Gaussian"], label="Method 3", linewidth=1.5)
        l4, = ax.plot(p4[:,x_idx], p4[:,y_idx], color=sci_colors["LOWESS"], label="Ours", linewidth=1.5)
        
        if collect_legend:
            handles.extend([l0, l1, l2, l3, l4])
            # 修改这里收集的标签文本
            labels_txt.extend(["Original", "Method 1", "Method 2", "Method 3", "Ours"])
    ...
    # 绘制子图
    draw(axes[0], 0, 1, "X Axis", "Y Axis", collect_legend=True)
    axes[0].set_title("(a) XY Plane", fontsize=12, fontweight='bold', pad=10)

    draw(axes[1], 1, 2, "Y Axis", "Z Axis")
    axes[1].set_title("(b) YZ Plane", fontsize=12, fontweight='bold', pad=10)

    draw(axes[2], 0, 2, "X Axis", "Z Axis")
    axes[2].set_title("(c) XZ Plane", fontsize=12, fontweight='bold', pad=10)

    # 顶部图例
    fig.legend(handles, labels_txt, loc='upper center', bbox_to_anchor=(0.5, 0.98), 
...
def visualize_folder(folder, dt=0.01, csv_path="stats_results.csv"):
    if not os.path.exists(folder):
        print(f"Folder '{folder}' not found. Creating a dummy file for testing.")
        os.makedirs(folder, exist_ok=True)
        with open(os.path.join(folder, "test_traj.txt"), "w") as f:
            t = np.linspace(0, 10, 100)
            x = np.sin(t)
            y = np.cos(t)
            z = t
            for i in range(len(t)):
                f.write(f"v{x[i]:.4f} {y[i]:.4f} {z[i]:.4f}\n")
    ...
    for i, filename in enumerate(files, 1):
        ...
        # LOWESS
        t0 = time.perf_counter()
        pts_lowess = smooth_lowess(pts_resampled, frac=0.03, it=0)
        ...
        plot_projection_comparison(pts_resampled, pts_sg, pts_spline, pts_gauss, pts_lowess, title)
...
if __name__ == "__main__":
    visualize_folder("top_curvature_regions", dt=0.01, csv_path="stats_results3.csv")